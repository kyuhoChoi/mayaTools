...
shelf_label = ''
shelf_cmds = []
shelf_cmds.append('import sceneAssembly')
shelf_cmds.append('sceneAssembly.ui()')
shelf_cmd   = '\n'.join(shelf_cmds)
shelf_anno  = u'스크립트 설명'
...
# ======================================
def uiContents():
    with pm.frameLayout(l='1. Catagory', cll=True, mw=3, mh=3, bs='etchedIn'):
        with pm.columnLayout(adj=True):
            with pm.rowLayout(nc=2, adj=2):
                pm.text(l=u'라벨 :', h=32, w=120, align='right')
                pm.button( l='Excute', c=pm.Callback( btn_sample ))
