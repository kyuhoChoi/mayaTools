...
def getNamespace(*args):
    if args:
        return getNamespace_from_nodeName(args[0])
    else :
...
def getNamespace_from_select():
    ...
    for _node in _sel:
        _namespace.append( getNamespace_from_nodeName(_node) )
...
def getNamespace_from_nodeName(_name):
    # _name = "Hello:MyNmae:This:Hand"
    _namespace = ''

    # string 형식이 아니면 나가버림
    if not (type(_name) == str or type(_name) == unicode):
        return

    _split = _name.split(':')
    # Result: ['Hello', 'MyNmae', 'This', 'Hand'] # 

    if len(_split)<2:
        return
    
    for _ns in _split[0:-1]: 
        _namespace += _ns+':'
...
def getHIKCharacterNode(*args):
    if args:
        return getHIKCharacterNode_from_nodeName(args[0])
    else :
...
def getHIKCharacterNode_from_select():
    ...
    for _node in _sel:
        ...
        _result = getHIKCharacterNode_from_nodeName(_node)
...
def getHIKCharacterNode_from_nodeName(_name):
    # namespace 확인
...
def getHIKProperty2State(*args):
    if args:
        return getHIKProperty2State_from_nodeName(args[0])
    else :
...
def getHIKProperty2State_from_select():
    ...
    for _node in _sel:
        ...
        _result = getHIKProperty2State_from_nodeName(_node)
...
def getHIKProperty2State_from_nodeName(_name):
    # namespace 확인
    _namespace = getNamespace(_name)
    _ls = []
...
def getRootParent(*args):
    if args:
        return getRootParent_from_nodeName(args[0])
    else :
...
def getRootParent_from_select():
    ...
    _root = []
    for _node in _sel:
        _result = getRootParent_from_nodeName(_node)
        if _result and not _result in _root:
            _root.append(_result)

    return _root[0]
    
def getRootParent_from_nodeName(_name):
    _root = []
    _transforms = []
