...
# 사용자정의변수.
win         = 'surfacingUI'
title       = 'Surfacing Tools'
labelWidth  = 200

shelf_icon  = iconPath + 'shelf_icon.png'  # 'pythonFamily.png'
shelf_label = ''
shelf_cmds = []
shelf_cmds.append('import surfacing')
shelf_cmds.append('surfacing.ui()')
shelf_cmd   = '\n'.join(shelf_cmds)
shelf_anno  = u'Surfacing 도구 모음'
...
# ======================================
def uiContents():
    with pm.frameLayout(l='Surface', cll=True, mw=3, mh=3, bs='etchedIn'):
        with pm.columnLayout(adj=True):
    ...
    with pm.frameLayout(l='Arnold Tools', cll=True, mw=3, mh=3, bs='etchedIn'):
        with pm.columnLayout(adj=True):
            with pm.rowLayout(nc=10):
                pm.text(l='ai Depth : ', w=labelWidth, align='right')
                pm.button(l='Open UI..', c=pm.Callback( btn_aiDepth ), w=160)
                ...
                pm.button(l='t', w=20, c=pm.Callback( excuteWindowFile, 'Ai_depth.mov') )

            with pm.rowLayout(nc=10):
                pm.text(l='ai Matte : ', w=labelWidth, align='right')
                pm.button(l='Open UI..', c=pm.Callback( btn_aiMatte ), w=160)
                #pm.button(l='d', w=20, c=pm.Callback( pm.launch, web="http://www.braverabbit.de/playground/dp=443") ) 
                #pm.button(l='t', w=20, c=pm.Callback( pm.launch, web="https://www.youtube.com/watch?v=YvNdYseSNFs") )
                pm.button(l='t', w=20, c=pm.Callback( excuteWindowFile, 'Ai_Matte.mov') )

            with pm.rowLayout(nc=10):
                pm.text(l='Arnold Add Attr (wip) : ', w= labelWidth, align='right')
                pm.button( l='Open UI..', w= 160, c=pm.Callback( btn_arnoldAddAttr ))

            with pm.rowLayout(nc=10):
                pm.text(l='Arnold Mesh Subdiv Render (wip): ', w= labelWidth, align='right')
